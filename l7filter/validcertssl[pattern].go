...
func init() {
	mypattern := Pattern{}
	mypattern.Name = "validcertssl"
	mypattern.RegularExpresion = "(?i)" + "^(.?.?\\x16\\x03.*\\x16\\x03|.?.?\\x01\\x03\\x01?.*\\x0b).*(thawte|equifax secure|rsa data security, inc|verisign, inc|gte cybertrust root|entrust\\.net limited)"
